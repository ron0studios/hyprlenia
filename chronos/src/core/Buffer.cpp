...
Buffer::Buffer() : m_id(0), m_count(0), m_type(GL_SHADER_STORAGE_BUFFER), m_initialized(false) {}

Buffer::Buffer(int count, GLenum type) : m_id(0), m_count(count), m_type(type), m_initialized(false) {}
...
void Buffer::setData(const std::vector<float>& data) {
    ...
    if (ptr) {
        size_t copySize = std::min(data.size(), static_cast<size_t>(m_count)) * sizeof(float);
...
void Buffer::bind(GLuint index) const {

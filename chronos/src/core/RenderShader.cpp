...
RenderShader::RenderShader() 
    : Shader(), m_vertexPath(""), m_fragmentPath(""), m_vao(0), m_vbo(0), m_ebo(0) {}

RenderShader::RenderShader(const std::string& vertexPath, const std::string& fragmentPath)
    : Shader(), m_vertexPath(vertexPath), m_fragmentPath(fragmentPath), m_vao(0), m_vbo(0), m_ebo(0) {}
...
void RenderShader::bindBuffer(const std::string& name, const Buffer& buffer, GLuint bindingPoint) const {
    GLint blockIndex = glGetProgramResourceIndex(m_id, GL_SHADER_STORAGE_BLOCK, name.c_str());

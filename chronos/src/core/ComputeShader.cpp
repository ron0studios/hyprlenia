...
ComputeShader::ComputeShader(const std::string& path) : Shader(), m_path(path) {}

void ComputeShader::init() {
    std::string source = readFile(m_path);
    if (source.empty()) {
        std::cerr << "ERROR: Failed to load compute shader: " << m_path << std::endl;
        return;
    }
...
void ComputeShader::bindBuffer(const std::string& name, const Buffer& buffer, GLuint bindingPoint) const {
